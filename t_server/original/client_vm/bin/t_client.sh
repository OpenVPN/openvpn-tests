...
if [ ! -x "$OPENVPN" ]
then
    echo "no (executable) $OPENVPN binary. FAIL." >&2
    exit 1
fi
...
do
    ...
    if [ -z "$expect_ifconfig4" ] || [ -z "$expect_ifconfig6" ]; then
        up="--setenv TESTNUM $SUF --setenv TOP_BUILDDIR . --script-security 2 --up ../bin/update_t_client_ips.sh"
    else
        up=""
    fi
    ...
    # we expected success, but OpenVPN did not start
	...
	if [ "$BRANCH" != "22" ] ; then
	    ...
	    echo -e "NOTE: differences between pre- and post-ifconfig/route ignored for 2.2 branch tests.\n"
	fi
    fi
    if [ "$fail_count" = 0 ] ; then
